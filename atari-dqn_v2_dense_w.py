...
def fit_batch(model, target_model, start_states, actions, rewards, next_states, is_terminal):
    ...
    loss = model.fit(
        ...
        epochs=1, batch_size=len(start_states), verbose=0, callbacks=callbacks
    )
    return loss
...
def run_game(mode='train'):
    ...
    if mode == 'run':
        # Only run the game
        OBSERVE = MAX_EPISODES * OBSERVATION  # Keep always observing. Run mode
        ...
        # Loading ALL stored weights of the current model
        print("Now we load Dense weights")
        # Initializing new empty model and loading the stored weights
        model_trained = atari_model(action_size)
        model_trained.load_weights("model.h5")
        # Getting each layer of the loaded model
        for i in xrange(0, len(model_trained.layers)):
            # Loading only Dense layers weights
            if type(model_trained.layers[i]).__name__ == 'Dense':
                weights = model_trained.layers[i].get_weights()
                model.layers[i].set_weights(weights)
        # Compiling the model
        model.compile(optimizer=RMSprop(lr=0.00025, rho=0.95, epsilon=0.01), loss=huber_loss)
        print("Dense Weight load successfully")
